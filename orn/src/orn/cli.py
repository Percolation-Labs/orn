...
def cmd_bench(args):
    """Full benchmark run on a known checkpoint; saves JSON to bench/results/."""
    import time
    ...
    for task, scores in results.items():
        summary[task] = {k.split(",")[0]: v for k, v in scores.items()
...
def main():
    ...
    # ── eval ──
    sp = sub.add_parser("eval", help="lm-eval benchmarks on a single checkpoint")
    sp.add_argument("--checkpoint", required=True,
                    help="Local path or known name (orn-v2-108m, orn-v3-605m, ...)")
    sp.add_argument("--tasks", default=None,
                    help="Comma-separated (default: DEFAULT_TASKS = the 7 paper tasks)")
    sp.add_argument("--limit", type=int, default=None, help="per-task cap for quick runs")
    ...
    sp.add_argument("--tasks", default=None,
                    help="Comma-separated (default: 7 paper tasks)")
