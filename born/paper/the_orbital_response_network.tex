...
The ORN is a genuinely different object from a standard transformer, and some of what looks like a limitation under transformer assumptions is an instance of the comparison not translating. We split this section into three kinds of remark: (A) comparisons and probes designed around per-layer transformers that do not carry over to ORN, (B) regimes where sharing the coupling rule is a real trade-off and could matter in real-world deployment, and (C) experimental gaps we intend to close.

\paragraph{(A) Comparisons that do not translate.}
\textit{Post-hoc weight surgery does not produce a usable model.} Replacing $W_Q, W_K$ in a frozen pretrained transformer with the fitted shared $\M$ from Section~\ref{sec:invariance} gives degraded perplexity. This is not a weakness of ORN; it is a consequence of how it works. The response heads ($W_V$, $W_O$) in an ORN co-train with a specific shared $\M$ and occupy a specific rotational gauge (Section~\ref{sec:arch}); substituting a shared $\M$ from a transformer that was trained in a different gauge puts the two halves out of register. End-to-end training is how ORN works; the existence proof in Section~\ref{sec:invariance} motivates the architecture, it is not a drop-in replacement recipe.

\textit{Per-layer interpretability probes read a different carrier.} A large literature on transformer interpretability reads information out of per-layer weights, or attributes circuits to specific layers (ROME-style edits, causal tracing by head, layer-specific probes~\cite{hewitt2019probe}). In an ORN the equivalent of ``layer 8's attention'' is not a thing; attention geometry is one shared object. Information that a transformer encodes in the differences between its per-layer $W_Q, W_K$ matrices lives, in ORN, in the residual-stream trajectory through a fixed coupling. Probes that read layer weights will see less in ORN because less is there; probes that read the residual stream should see more (Section~\ref{sec:discussion}, preliminary). This is a reframing of what interpretability means on this architecture, not a deficiency of the architecture.

\textit{External baselines are not compute-matched.} GPT-2 Small and SmolLM-135M saw 37--75$\times$ more training tokens than \orn{}-V2. Table~\ref{tab:benchmarks} should be read as ``the architectural substitution does not hurt at this scale'', not as per-token superiority. A matched-tokens ORN-vs-transformer head-to-head on the same 8B FineWeb-Edu run is in progress.

\paragraph{(B) Real trade-offs of sharing the coupling.}

\textit{Exact lexical retrieval at long range.} A standard transformer can devote some of its $L$ coupling matrices to long-range lexical patterns and others to local syntax; per-layer specialisation is the mechanism. An ORN has one coupling rule everywhere, and any long-range exact-recall behaviour has to be built across the residual-stream trajectory rather than at a specialised attention layer. LAMBADA is the canonical benchmark for this capability, and V2's LAMBADA number (26.2\%) is clearly below its data-rich baseline. At our current scale we cannot cleanly separate ``less data'' from ``structural cost'' because both confounders are present, but the structural piece is real. In a practical setting where exact lexical retrieval at long context matters (code completion, templated generation, faithful citation), this is the axis to watch.

\textit{Tasks where the coupling rule is not universal.} The ORN's thesis is that language coupling is approximately the same at every depth and that transformers converge to this property during training. If a domain exists where coupling genuinely is not universal, where some depths want a structurally different coupling rule, ORN caps there. We do not have a clean counter-example in natural-language tasks today. Plausible candidates include mixed symbolic / sub-symbolic work, multi-phase planning tasks, and sufficiently structured code, where depth-specialised attention plausibly matters more than it does in ordinary text. This is a hypothesis for future experiments, not a demonstrated failure mode.

\textit{Research patterns that depend on per-layer weight edits.} If your downstream programme requires localised weight edits (ROME, ReFT, per-layer LoRA adapters targeting specific attention patterns), ORN is a different object. The equivalent operation lives in the response heads or in residual-stream interventions, which is a different research programme with, we suspect, several open questions of its own.

\paragraph{(C) Experimental gaps we intend to close.}
(1) Matched-compute baseline (same-size transformer trained on the same 8B FineWeb-Edu tokens as V2, and a separate match at 3B for V3); (2) seed variance on Table~\ref{tab:scaling}, Table~\ref{tab:moe}, and the small multimodal table; (3) full-harness V3-605M benchmark row (currently produced at \texttt{limit=200} on MPS, pending a full run); (4) spectral-invariance coverage beyond the GPT-2 family, SmolLM-135M, and Qwen-2.5 0.5B, to Llama, Mistral, Qwen3, DeepSeek, and SmolLM2; (5) the syntactic-separation measurement (Section~\ref{sec:discussion}) on a standard benchmark such as BLiMP~\cite{warstadt2020blimp}. None of these require a change to the architecture; all will appear in subsequent revisions.
