...
% ======================================================================
\begin{abstract}
We ask what happens if a transformer's per-layer query-key coupling is replaced by a single shared bilinear form, motivated by the observation that the coupling's eigenspectrum is nearly identical across layers of pretrained transformers (mean pairwise spectral correlation $0.94$--$0.99$ across GPT-2 Small/Medium/Large, SmolLM-135M, and Qwen-2.5 0.5B) even though the raw matrices are orthogonal (cosine $\sim 0.005$).
The \textbf{Orbital Response Network} (\orn{}) replaces $2L$ per-layer $W_Q, W_K$ matrices with one shared $\M = AB^\top$ and keeps $W_V$, $W_O$, and the FFN per-layer.
This paper's claim is narrow: the substitution does not hurt.
At 108M parameters on 8B tokens of FineWeb-Edu (a single RTX 4090, \textasciitilde\$8), \orn{}-V2 comes within a point of GPT-2 Small and SmolLM-135M on average across eight zero-shot benchmarks despite 37--75$\times$ less training data, using 48$\times$ fewer coupling parameters; a frozen $\M$ transferred from one dataset to another recovers 98.6\% of from-scratch validation loss, and the emergent $\M$ crystallises to a 5--8 dimensional manifold matching our pretrained-model analysis.
A larger \orn{}-V3 at 605M parameters ($d{=}2048$, $L{=}32$, $8\times$ wide shared FFN~\cite{onewideffn2023}) reached val loss 2.82 on 3B tokens of FineWeb-Edu on a single A100 (\textasciitilde40 hours, \textasciitilde\$394); benchmark evaluation of V3 is forthcoming.
Code, published checkpoints (\texttt{mr-saoirse/orn-v2-108m} and \texttt{mr-saoirse/orn-v3-605m} on HuggingFace), and all experimental scripts are at the repository above; most diagnostics reproduce on a laptop in minutes.
\end{abstract}
