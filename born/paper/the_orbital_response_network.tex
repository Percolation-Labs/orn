...
(1) \textbf{The external baselines in Table~\ref{tab:benchmarks} are not compute-matched.} GPT-2 Small and SmolLM-135M saw 37--75$\times$ more training tokens than ORN-V2. The results there should be read as ``the architectural substitution does not hurt at 108M / 8B'', not as a claim of per-token superiority. A matched ORN-vs-transformer-on-same-8B-tokens head-to-head is the correct follow-up and is in progress.
(2) Most within-paper numbers are single runs at each configuration. Seed variability is not reported and error bars on Table~\ref{tab:scaling} and Table~\ref{tab:moe} are missing. Where the gap between ORN and transformer is less than a few percent (which is most of the time), the result should be read as ``tracks'' rather than ``beats''.
(3) ORN-V3-605M has a validation loss but no zero-shot benchmark numbers in this version; Section~\ref{sec:results} flags this explicitly and the benchmark run is under way.
(4) Post-hoc surgery on a pretrained transformer (replacing $W_Q, W_K$ with the fitted shared $\M$ from Section~\ref{sec:invariance}) does not produce a usable model; shared coupling has to be trained in.
(5) The spectral-invariance measurement covers the GPT-2 family, SmolLM-135M, and Qwen-2.5 0.5B. Llama, Mistral, and the larger SmolLM2 are not yet included; broader coverage would strengthen the claim.
(6) The syntactic-separation result (Section~\ref{sec:discussion}) is based on a small curated pair set, not a standard benchmark such as BLiMP. We flag this as preliminary.
(7) The multimodal experiments use 7M-parameter models on compressed representations per modality, not full high-resolution image / audio models; the image underperformance may be specific to that regime.
