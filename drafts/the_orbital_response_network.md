...
Before the next section, a caveat about how to read all of the above. I am not trying to claim that ORN is a better language model than whatever you are currently using, or that these benchmark numbers mean we are on the frontier. They do not and we are not. What the numbers show is narrower and, I think, more interesting: the architectural change does not hurt. You can take a standard transformer, pull out most of the coupling parameters, make a single shared object do the job across every layer, train on considerably less data than the baselines were trained on, and end up somewhere in the same region of benchmark space. That is the engineering lesson, and it is a permissive one.

What I find more interesting than the benchmark numbers is what the shared backbone enables downstream, which is not something I can prove in this article but which I think is where the real pay-off of this decomposition will be. A few concrete directions I am actively working on or thinking about:

- **Mixture of experts without the backbone cost.** If every expert in an MoE shares the same *M* and (in V3) the same wide FFN, then a hundred experts do not mean a hundred copies of the backbone. Each expert specialises only in its response heads, and the shared part stays one object. The parameter accounting for MoE gets much friendlier, and the interpretability argument that an expert is a "variation on a theme" becomes literal rather than aspirational.
- **Fine-tuning as response-only adaptation.** A frozen backbone (shared *M* plus the wide shared FFN) plus freshly trained response heads recovers 98.6% of full training quality, which is the frozen-M transfer result from earlier. That number says fine-tuning to a new domain is essentially a response-head problem, not a coupling-geometry problem. The practical implication is that domain fine-tuning on an ORN should be significantly cheaper than the same operation on a dense transformer with per-layer coupling, because there are fewer moving parts to retrain.
- **A shared object you can actually reason about.** Across layers and across training runs, the *M* that emerges is a low-rank, bounded, inspectable thing. It has eigenvectors you can plot, a rank you can track during training, and a spectral signature that replicates across model families. Whatever a "world model" for language turns out to mean, it is going to have to be an object like that rather than a diffuse mix of per-layer parameters. ORN does not solve interpretability, but it gives interpretability a small, concrete target to aim at instead of asking what a million distinct weights happen to compute between them. That is where I think the architectural move earns its keep, and where the next round of experiments is pointed.
- **Long context as a rotation around the shared block.** This is the direction I think is most under-explored. The observation that all *L* layers of a trained transformer have the same *M* up to spectral correlation is, mathematically, a statement that *M_l* is orthogonally conjugate to a single shared *M*: there is a per-layer rotation *R_l* such that *M_l* ≈ *R_l*ᵀ *M* *R_l*. ORN sets *R_l* = identity at every layer and absorbs the variation into the response heads. RoPE does the structurally identical thing along the position axis instead of the depth axis: it rotates *Q* and *K* by an angle that depends on token index, so position enters the dot product as a relative rotation of the same shared *Q*·*K*ᵀ form. Two rotation axes (depth, position), one invariant object. Composed, they trace a helix around *M*, with each layer-and-position pair reading the shared coupling at its own angle. The two rotations do not commute in general, and the non-commutativity is exactly the kind of phase structure you would want for resolving long-range hierarchical dependencies. Concretely, this points at a couple of cheap experiments we have not yet run: fit per-layer rotations to a pretrained transformer's *M_l* sequence and check whether they decompose into log-spaced 2D blocks the way RoPE's position rotations do; train an ORN variant with a depth-RoPE applied to *Q* and *K* before the standard position-RoPE, with zero added parameters, and see whether the failure modes that pure shared-coupling cannot express (the BLiMP island constraints, the long-distance NPI cases) get unlocked by giving each depth its own reading angle. None of this is a claim that ORN already solves long context. It is a claim that the rotational frame ORN sets up makes long-context engineering composable with the rest of the architecture in a way that per-layer coupling does not.
